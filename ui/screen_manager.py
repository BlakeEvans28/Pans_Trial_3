...
class Screen:
    ...
    def _draw_wrapped_text(
        ...
        """Draw plain wrapped text clipped to a fixed number of lines."""
        lines = self._wrap_text_lines(text, font, rect.width, max_lines)
        for index, line in enumerate(lines[:max_lines]):
            line_surface = font.render(line, True, color)
            line_rect = line_surface.get_rect()
            if align == "center":
                line_rect.midtop = (rect.centerx, rect.y + index * line_height)
            else:
                line_rect.topleft = (rect.x, rect.y + index * line_height)
...
class RoomSelectionScreen(Screen):
    """Localhost room selection and waiting screen."""

    def __init__(self, window: "GameWindow", room_client):
        super().__init__(window)
        ...
        self.room_code_entry = None
        self.create_button = None
        self.join_button = None
        self.back_button = None
        self.panel_rect = pygame.Rect(0, 0, 1, 1)
        self.player_name_rect = pygame.Rect(0, 0, 1, 1)
        self.room_code_rect = pygame.Rect(0, 0, 1, 1)
        self.button_rects: dict[str, pygame.Rect] = {}
        self.hovered_button = None
        self.status_message = "Enter a room code or create one. The match begins when two players enter the same room."
        self.room_players: list[dict] = []
        self.joined_room_code: str | None = None
    ...
    def _refresh_fonts(self) -> None:
        """Refresh room-screen fonts for the current scale."""
        self.title_font = self._get_game_font(self.font_size(58, 34))
        self.body_font = self._get_game_font(self.font_size(28, 19))
        self.small_font = self._get_game_font(self.font_size(22, 15))

    def _create_ui(self) -> None:
        """Create room screen entry fields and hidden button controls."""
        self.player_name_entry = pygame_gui.elements.UITextEntryLine(
            relative_rect=pygame.Rect((0, 0), (1, 1)),
            manager=self.ui_manager,
            object_id="room_player_name",
        )
        self.room_code_entry = pygame_gui.elements.UITextEntryLine(
            relative_rect=pygame.Rect((0, 0), (1, 1)),
            manager=self.ui_manager,
            object_id="room_code_entry",
        )
        self.create_button = pygame_gui.elements.UIButton(
            relative_rect=pygame.Rect((0, 0), (1, 1)),
            text="Create Room",
            manager=self.ui_manager,
            object_id="room_create_button",
        )
        self.join_button = pygame_gui.elements.UIButton(
            relative_rect=pygame.Rect((0, 0), (1, 1)),
            text="Join Room",
            manager=self.ui_manager,
            object_id="room_join_button",
        )
        self.back_button = pygame_gui.elements.UIButton(
            relative_rect=pygame.Rect((0, 0), (1, 1)),
            text="Back",
            manager=self.ui_manager,
            object_id="room_back_button",
        )
        self.player_name_entry.set_text("Player")

    def _layout_ui(self) -> None:
        """Lay out the panel, entry fields, and hidden room buttons."""
        self.panel_rect = pygame.Rect(
            self.scale_x(180, 22),
            self.scale_y(108, 28),
            self.window.WINDOW_WIDTH - 2 * self.scale_x(180, 22),
            self.window.WINDOW_HEIGHT - 2 * self.scale_y(108, 28),
        )
        if self.is_compact_layout():
            self.panel_rect = pygame.Rect(
                self.scale_x(22, 10),
                self.scale_y(18, 10),
                self.window.WINDOW_WIDTH - 2 * self.scale_x(22, 10),
                self.window.WINDOW_HEIGHT - 2 * self.scale_y(18, 10),
            )

        content_rect = self._get_stone_content_rect(
            self.panel_rect,
            extra_x=self.scale_x(10, 6),
            extra_top=self.scale_y(12, 8),
            extra_bottom=self.scale_y(10, 6),
        )
        field_height = self.scale_y(44, 34)
        field_gap = self.scale_y(18, 12)
        label_gap = self.scale_y(8, 5)
        first_field_y = content_rect.y + self.scale_y(102, 74)

        self.player_name_rect = pygame.Rect(
            content_rect.x,
            first_field_y,
            content_rect.width,
            field_height,
        )
        self.room_code_rect = pygame.Rect(
            content_rect.x,
            self.player_name_rect.bottom + field_gap + label_gap + self.scale_y(18, 12),
            content_rect.width,
            field_height,
        )

        button_gap = self.scale_x(18, 10)
        button_width = (content_rect.width - button_gap) // 2
        button_height = self._get_wood_icon_height_for_width(button_width)
        create_rect = pygame.Rect(
            content_rect.x,
            self.room_code_rect.bottom + self.scale_y(44, 26),
            button_width,
            button_height,
        )
        join_rect = pygame.Rect(
            create_rect.right + button_gap,
            create_rect.y,
            button_width,
            button_height,
        )
        back_width = min(content_rect.width, self.scale_x(280, 190))
        back_rect = pygame.Rect(
            content_rect.centerx - back_width // 2,
            content_rect.bottom - self._get_wood_icon_height_for_width(back_width),
            back_width,
        ...
        self.player_name_entry.set_relative_position((self.player_name_rect.x, self.player_name_rect.y))
        self.player_name_entry.set_dimensions((self.player_name_rect.width, self.player_name_rect.height))
        self.room_code_entry.set_relative_position((self.room_code_rect.x, self.room_code_rect.y))
        self.room_code_entry.set_dimensions((self.room_code_rect.width, self.room_code_rect.height))
    ...
    def _hide_all_elements(self) -> None:
        """Hide all room-screen controls."""
        for element in [
            self.player_name_entry,
            self.room_code_entry,
            self.create_button,
            self.join_button,
            self.back_button,
        ]:
    ...
    def _set_waiting_controls_enabled(self, enabled: bool) -> None:
        """Enable or disable room-entry controls while waiting."""
        method_name = "enable" if enabled else "disable"
        for element in [self.player_name_entry, self.room_code_entry, self.create_button, self.join_button]:
            getattr(element, method_name)()

    def prepare_for_new_session(self) -> None:
        """Reset the room screen when entering from the menu or after a finished match."""
        self.room_client.close()
        self.joined_room_code = None
        self.waiting_for_start = False
        self.room_players = []
        self.status_message = "Enter a room code or create one. The match begins when two players enter the same room."
        self.room_code_entry.set_text("")
        self._set_waiting_controls_enabled(True)

    def apply_network_message(self, message: dict) -> None:
        """Update room-screen status from one network message."""
        message_type = message.get("type")
        if message_type == "room_joined":
            self.joined_room_code = message.get("room_code")
            self.room_players = list(message.get("players", []))
            self.waiting_for_start = True
            player_id = message.get("player_id")
            self.status_message = (
                f"You entered room {self.joined_room_code} as Player {player_id + 1}. "
                "Waiting for the second player."
            )
            self._set_waiting_controls_enabled(False)
            return

        if message_type == "room_update":
            self.room_players = list(message.get("players", []))
            self.waiting_for_start = message.get("status") != "active"
            self.status_message = message.get("message", self.status_message)
        ...
        if message_type == "game_start":
            self.room_players = list(message.get("players", []))
            self.waiting_for_start = False
            self.status_message = "Both players are here. Launching the game."
            return

        if message_type in {"room_closed", "disconnected"}:
            self.joined_room_code = None
            self.waiting_for_start = False
            self.room_players = []
            self.status_message = message.get("message", "The room connection was lost.")
            self._set_waiting_controls_enabled(True)
            return

        if message_type == "error":
            self.waiting_for_start = False
            self.status_message = message.get("message", "Room connection failed.")
            self._set_waiting_controls_enabled(True)

    def handle_events(self, event: pygame.event.Event) -> bool:
        """Handle room button clicks."""
        if event.type == pygame.MOUSEMOTION:
            self.hovered_button = self._room_action_at(event.pos)
            return False

        if event.type == pygame.MOUSEBUTTONDOWN:
            action = self._room_action_at(event.pos)
            if action is not None:
                return self._activate_room_action(action)
            return False
        ...
        action = key_by_element.get(event.ui_element)
        if action is None:
            return False
        return self._activate_room_action(action)

    def _room_action_at(self, pos: tuple[int, int]) -> str | None:
        """Return the room-screen button under the pointer."""
        for key, rect in self.button_rects.items():
            if self._point_hits_wood_icon(rect, pos):
                return key
        return None

    def _activate_room_action(self, action: str):
        """Apply one room-screen action."""
        if action == "back":
            self.room_client.close()
            self.joined_room_code = None
            self.waiting_for_start = False
        ...
        room_code = self.room_code_entry.get_text().strip().upper()
        player_name = self.player_name_entry.get_text().strip() or "Player"
        if action == "create":
            if not room_code:
                room_code = generate_room_code()
                self.room_code_entry.set_text(room_code)
        elif not room_code:
            self.status_message = "Enter a room code before joining."
            return True

        if not self.room_client.join_room(room_code, player_name):
            self.status_message = "Couldn't reach the localhost room server. Try again in a moment."
            self.waiting_for_start = False
            self._set_waiting_controls_enabled(True)
            return True

        self.joined_room_code = room_code
        self.waiting_for_start = True
        self.room_players = []
        self.status_message = f"Entering room {room_code}..."
        self._set_waiting_controls_enabled(False)
        return True

    def update(self, time_delta: float) -> None:
        """Room screen has no timed logic."""
        pass

    def render(self, surface: pygame.Surface) -> None:
        """Render the localhost room screen."""
        self._render_screen_background(surface, (18, 22, 30))
        self._render_stone_panel(surface, self.panel_rect, dim_alpha=26, shadow_alpha=58)
        content_rect = self._get_stone_content_rect(
            self.panel_rect,
            extra_x=self.scale_x(10, 6),
            extra_top=self.scale_y(12, 8),
        ...
        title = self.title_font.render("Local Room", True, (240, 236, 214))
        surface.blit(title, title.get_rect(midtop=(content_rect.centerx, content_rect.y)))

        intro_rect = pygame.Rect(
            content_rect.x,
            content_rect.y + self.scale_y(42, 28),
            content_rect.width,
            self.scale_y(44, 32),
        )
        self._draw_wrapped_text(
            surface,
            "Create a localhost room or join one by code. The match starts as soon as both players are connected.",
            self.small_font,
            (214, 214, 214),
            intro_rect,
            max(self.scale_y(18, 14), self.small_font.get_linesize()),
            2,
            align="center",
        )

        player_label = self.body_font.render("Player Name", True, (238, 232, 202))
        surface.blit(player_label, (self.player_name_rect.x, self.player_name_rect.y - self.scale_y(28, 20)))
        room_label = self.body_font.render("Room Code", True, (238, 232, 202))
        surface.blit(room_label, (self.room_code_rect.x, self.room_code_rect.y - self.scale_y(28, 20)))

        summary_top = self.button_rects["create"].bottom + self.scale_y(28, 18)
        room_text = self.joined_room_code if self.joined_room_code else "Not connected"
        room_surface = self.body_font.render(f"Active Room: {room_text}", True, (246, 238, 204))
        surface.blit(room_surface, (content_rect.x, summary_top))

        player_count = len(self.room_players)
        count_surface = self.small_font.render(f"Players in room: {player_count}/2", True, (226, 224, 214))
        surface.blit(count_surface, (content_rect.x, summary_top + self.scale_y(30, 22)))

        player_list_y = summary_top + self.scale_y(58, 40)
        if self.room_players:
            for player in self.room_players:
                player_line = self.small_font.render(
                    f"Player {player['player_id'] + 1}: {player['player_name']}",
                    True,
                    (220, 216, 204),
                )
                surface.blit(player_line, (content_rect.x, player_list_y))
                player_list_y += self.scale_y(24, 18)

        status_rect = pygame.Rect(
            content_rect.x,
            max(player_list_y + self.scale_y(12, 8), self.button_rects["back"].y - self.scale_y(90, 66)),
            content_rect.width,
            self.scale_y(70, 54),
        )
        self._draw_wrapped_text(
        ...
        self.hovered_button = self._room_action_at(pygame.mouse.get_pos())
        self._render_wood_button(surface, self.button_rects["create"], "Create Room", self.hovered_button == "create")
        self._render_wood_button(surface, self.button_rects["join"], "Join Room", self.hovered_button == "join")
        self._render_wood_button(surface, self.button_rects["back"], "Back", self.hovered_button == "back")
    ...
    def on_resize(self) -> None:
        """Refresh room layout after a resize."""
        self._refresh_fonts()
        self._layout_ui()
